...
func Test_TerminateCall(t *testing.T) {
	...
	} else if inst, err := NewDecodedInstruction(InstructionGroupMisc, MiscOpSystemCall, SystemCommandTerminate, 0, 0); err != nil {
		t.Errorf("Couldn't create decoded instruction: %s", err)
	} else if err := core.InvokeExecution(inst); err != nil {
